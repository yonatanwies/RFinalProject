...
```{r}
#I added library(gtools) to use function "permutations"
find_cycles <- function(perm, k){
  X <- order(perm, decreasing = FALSE)
  res <- vector("list")
  for(i in X){
    temp <- c(i)
    if(k==1){
  ...
  res <- res[!sapply(res, is.null)]  # Remove NULL entries

  
  return(res)
  }}
res <- find_cycles(c(3,4,1,5,2),2)
res
# Remove duplicate lists
unique_res <- res[!duplicated(sapply(res, toString))]

# Print the unique lists
for (lst in unique_res) {
  print(lst)
}
find_cycles(c(3,4,1,5,2),3)
```
...
```{r}
find_all_cycles<-function(perm,k){
  result<-c()
  X<- order(perm,decreasing = FALSE)
  for(i in X){
    temp<-c(i)
