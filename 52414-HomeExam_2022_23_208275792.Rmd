...
```{r}
find_probability_p12 <- function(perm) {
  cycles <- find_all_cycles(perm)
  
  # Check if 1 and 2 are in the same cycle
  same_cycle<- (1 %in% cycles && 2 %in% cycles)  
  if (same_cycle) {
    return(1)
  } else {
    return(0)
...
n <- 100
simulations <- 10000
sample_permutations <- replicate(simulations, sample(n))

# Estimate probability p12
p12 <- mean(sapply(sample_permutations, find_probability_p12))
p12
