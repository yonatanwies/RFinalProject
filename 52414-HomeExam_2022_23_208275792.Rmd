...
for (i in 1:simulations) {
  alice_result <- bob_given_alice(ndeck, nhands)
  if (!is.na(alice_result)) {
    count <- count + alice_result
  } else {
    rejected <- rejected + 1
  }
}

p_b_given_a <- count / (simulations - rejected)
...
combined_df<-merge.data.frame(df,economic_df,by=intersect(df$Country,economic_df$Country.Name))
unique(df$Country) %in% unique(economic_df$Country.Name)
class(economic_df$X2018..YR2018.)
```
