...
neighboring_EMRO <- neighboring_EMRO[-c(1,2,3,8,10,11,14,17,18,20,21)]
ggplot(df[df$Country %in% neighboring_EMRO,], aes(x = Date, y = Cumulative_cases, color = Country)) + geom_line() +
...
ggplot(df[df$Country %in% neighboring_EMRO,], aes(x = Date, y = Cumulative_deaths, color = Country)) + geom_line() +
...
combined_df<-left_join(df,economic_df,by=df$Country)
economic_df <- dcast(economic_df, `Country` ~  `Series.Name`, value.var = "X2018..YR2018.", fun.aggregate = sum)
names(economic_df)
economic_df$Country.Name[economic_df$Country ==  "Egypt, Arab Rep."] <- "Egypt"
economic_df$Country.Name[economic_df$Country ==  "Syrian Arab Republic"] <- "Syria"
economic_df$Country.Name[economic_df$Country ==  "Iran, Islamic Rep."] <- "Iran"
